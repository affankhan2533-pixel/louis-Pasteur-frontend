...
import { Key, Mail, Sparkles, Lock, ShieldAlert } from 'lucide-react';
  ...
  const handleAuthSubmit = async (e) => {
    ...
    setTimeout(async () => {
      try {
        ...
        setError("Cannot reach the server. Please try again or use Google Sign-In.");
      } finally {
        setLoading(false);
      }
    }, 800);
      ...
      <div className="hidden lg:block lg:col-span-6 relative bg-black">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/30 to-transparent flex flex-col justify-end p-16">
          ...
          <p className="text-[11px] text-gray-400 mt-2 leading-relaxed max-w-sm">
            Access your synchronized AI profile metrics and complete customized sizing configurations.
          </p>
        </div>
      </div>
      ...
      <div className="lg:col-span-6 flex items-center justify-center p-8 md:p-16">
        <div className="w-full max-w-[420px] flex flex-col gap-6">
          <div className="text-center flex flex-col items-center">
            <span className="text-[9px] tracking-[0.25em] text-gold font-bold uppercase">
              {isAdminMode ? "ADMIN CONTROL CENTER" : "SECURE PORTAL"}
            </span>
            <h2 className="text-2xl font-editorial font-bold text-primary mt-1 uppercase">
              {isAdminMode ? "ADMIN PORTAL LOG IN" : "SIGN IN TO PROFILE"}
            </h2>
            <p className="text-[11px] text-gray-400 mt-1">
              {isAdminMode ? "Enter authorization passcode to connect configuration console." : "Access your saved sizing data and order histories."}
            </p>
          </div>
          ...
          {!isAdminMode && (
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] text-gray-400 font-semibold uppercase">OR EMAIL LOGIN</span>
              <div className="flex-grow h-px bg-luxury-border" />
            </div>
          )}
          ...
          <form onSubmit={handleAuthSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] text-gray-400 font-bold uppercase">
                {isAdminMode ? "Admin Email Reference" : "Email Address"}
              </label>
              <div className="relative">
                <Mail size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                <input 
                  type="email" 
                  placeholder={isAdminMode ? "admin@louispasteur.com" : "aurelius@louispasteur.com"}
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="w-full text-[12px] pl-9 pr-3 py-2.5 border border-luxury-border rounded focus:outline-none focus:border-gold animate-fade"
                  required
                />
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] text-gray-400 font-bold uppercase">
                {isAdminMode ? "Admin Passcode" : "Password"}
              </label>
              <div className="relative">
            ...
            >
              {loading ? (
                <>
                  ...
                  {isAdminMode ? 'VERIFYING SECURITY SIGNATURE...' : 'AUTHENTICATING...'}
                </>
              ) : (isAdminMode ? 'SECURE ADMIN SIGN IN' : 'SECURE SIGN IN')}
            </button>
          </form>

          <div className="border-t border-luxury-border/60 pt-4 flex flex-col gap-3 text-center">
            {!isAdminMode && (
              <Link 
                href="/signup"
                className="text-[11px] text-gold hover:text-primary transition-colors font-semibold"
              >
                Need a new profile? Create an Atelier ID
              </Link>
            )}
            ...
            <button
              ...
              onClick={() => {
                ...
                setError('');
                setEmail(isAdminMode ? '' : 'admin@louispasteur.com');
                setPassword('');
              }}
              className="text-[10px] tracking-wider text-gray-400 hover:text-gold uppercase font-bold transition-colors"
            >
              {isAdminMode ? "← Back to standard login" : "Administrator Portal Verification →"}
            </button>
          </div>
        </div>
      </div>
